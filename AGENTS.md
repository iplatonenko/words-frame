...
- умеет shuffle
- умеет показывать весь список слов в модалке со скроллом
...
- В панели показывается имя активного CSV

### Управление

- Загрузить CSV
- Clear CSV (очистка загруженного CSV из приложения)
- Старт / Пауза
- Назад / Вперёд
- Shuffle
- Show List (модалка со скроллом и полным списком)
...
- Режим табло:
  ...
  - выход из табло: **долгий тап (~1000мс)**
...
- После импорта CSV сохранять:
  ...
  - имя активного CSV: `localStorage["wf_csv_name_v1"]`
- При старте:
...
- Проверить, что после изменений не сломаны `localStorage` ключи: `wf_words_v1`, `wf_index_v1`, `wf_interval_v1`, `wf_board_v1`, `wf_swap_v1`, `wf_theme_v1`, `wf_size_known_v1`, `wf_size_learning_v1`, `wf_csv_name_v1`.
...
- Swap Sides работает и сохраняется
- Known/Learning size настраиваются вручную и сохраняются
- Show List открывает полный список в модалке со скроллом
- Название активного CSV отображается в панели и сохраняется
